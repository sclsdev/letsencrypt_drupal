...
deploy_cert() {
    ...
    if [ -z ${CERT_DEPLOY_ENVIRONMENT_UUID+x} ]
      then
        ...
        RESULT=$(php $CURRENT_DIR/../acquia_cloud_cert_deployment/cert_deploy.php "${CERT_DEPLOY_ENVIRONMENT_UUID}" "${KEYFILE}" "${FULLCHAINFILE}" "${CHAINFILE}" "${TIMESTAMP} "${DOMAIN}" --activate --label-prefix "letsencrypt_drupal" 2>&1)
        if [ $? -eq 0 ]
        then
